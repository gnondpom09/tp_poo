...
if (isset($_POST['ajouter'])) { //si fichier sélectionné
   ...
   // echo  "<br/>".$type;
    ...
    //$id = $bdd->exec('SELECT users.id FROM users,datas where users.id=datas.auteur_id and users.nom="moi"');
    $rep = $bdd->query('SELECT users.id FROM users where users.nom="moi"');
    $id=($rep->fetch())['id'];
    echo '<br/>'.$id;
